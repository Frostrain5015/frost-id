<script lang="ts">
	...
	// Investory brand icon (simplified favicon — purple crystalline geometry)
	const investorySvg = `<svg fill="none" viewBox="0 0 48 46" xmlns="http://www.w3.org/2000/svg"><path fill="#863bff" d="M25.946 44.938c-.664.845-2.021.375-2.021-.698V33.937a2.26 2.26 0 0 0-2.262-2.262H10.287c-.92 0-1.456-1.04-.92-1.788l7.48-10.471c1.07-1.497 0-3.578-1.842-3.578H1.237c-.92 0-1.456-1.04-.92-1.788L10.013.474c.214-.297.556-.474.92-.474h28.894c.92 0 1.456 1.04.92 1.788l-7.48 10.471c-1.07 1.498 0 3.579 1.842 3.579h11.377c.943 0 1.473 1.088.89 1.83L25.947 44.94z"/></svg>`;

	// Check if this is Investory — use its real brand icon
	const isInvestory = $derived(data.clientName.toLowerCase().includes('investory'));
			...
			<div class="dual-brand">
				...
				<div class="brand-block brand-block--app">
					<div class="app-icon-wrap">
						{#if isInvestory}
							<div class="investory-icon" aria-hidden="true">{@html investorySvg}</div>
						{:else}
...
<style>
	...
	.investory-icon { width: 46px; height: 46px; display: flex; align-items: center; justify-content: center; }
	.investory-icon :global(svg) { width: 100%; height: 100%; display: block; filter: drop-shadow(0 0 6px rgba(134,59,255,0.3)); }
	.app-icon {
