...
export function storePending(
	authRequest: AuthorizationRequest,
	oauthParams: string | null = null,
	nonce: string | null = null
): string {
	const id = nanoid(24);
	store.set(id, { authRequest, oauthParams, nonce, expiresAt: Date.now() + 10 * 60 * 1000 });
	return id;
}
...
export function getPendingNonce(id: string): string | null {
	const entry = store.get(id);
	if (!entry) return null;
	if (Date.now() > entry.expiresAt) {
		store.delete(id);
